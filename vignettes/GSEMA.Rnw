...
\begin{itemize}
    ...
    \item \emph{threshold}:A number that indicates the threshold to eliminate a gene set. For a eliminate a gene set is necessary that the mean for both groups are less than the threshold. If  threshold = "sd" the threshold will be the standard deviation of the gene set. If the standard deviation is greater than 1, the threshold will be 1. The default value is "sd".
\end{itemize}

<<>>=
objectMApathSim <- filteringPaths(objectMApathSim, threshold = "sd")
@
